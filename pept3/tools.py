...
def read_m_r_ions(save2, sample_size=None):
    m_r = []
    m_i_delta = []
    m_i = []
    table = pd.read_csv(save2, sep='\t', header=None, nrows=sample_size)
    m_r = table[0].apply(lambda x:eval(x)).to_list()
    m_i_delta = table[1].apply(lambda x:eval(x)).to_list()
    m_i = table[2].apply(lambda x:eval(x)).to_list()
