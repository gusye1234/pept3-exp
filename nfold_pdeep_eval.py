...
def overlap_analysis(tab1, tab2, testfdr=0.01, compare=["prosit_combined", "prosit_combined"]):
    baseline = "sa"
...
def eval_fdr(models, msms_file, raw_dir, save_tab, fdr_threshold=0.1, show_fdr=[0.1, 0.01, 0.001, 0.0001], 
             sample_size=None, need_all=False, irt_model=None, id2selects=None, pearson=False, overwrite=True):
    ...
    for name in totest:
        ...
        if overwrite:
            ...
            print(f"read {name} from history")
...
if __name__ == "__main__":
    ...
    #     save_tab2 = f"/data2/yejb/prosit/figs/figure6/{which}/percolator/try/{frag_model}/pdeep_3fold"
    ...
    #     models, id2selects = finetune.pdeep3_few_shot_score(
    ...
    #           irt_model=prosit_irt, sample_size=sample_size, id2selects=id2selects, pearson=if_pearson, overwrite=False).to_string())
    ...
    # for which in ["trypsin", 'lysc']:
        ...
        save_tab2 = f"/data2/yejb/prosit/figs/fig235/{which}/percolator_up/try/{frag_model}/pdeep_3fold_{q_threshold}"
        if not os.path.exists(save_tab1):
            os.mkdir(save_tab1)
        if not os.path.exists(save_tab2):
            os.mkdir(save_tab2)
        ...
        # analysis_dict[which] = overlap_analysis_peptides(save_tab1, save_tab2)
        analysis_dict[which] = overlap_analysis_peptides(save_tab2, f"/data2/yejb/prosit/figs/fig235/{which}/percolator_up/try/{frag_model}/finetuned_3fold_{q_threshold}")
    print(analysis_dict)
#     print("-------------------------------")
#     print("Davis")
...
#     save_tab2 = f"/data2/yejb/prosit/figs/figure5/percolator/try/{frag_model}/pdeep_3fold_{q_threshold}"
#     if not os.path.exists(save_tab2):
#         os.mkdir(save_tab2)
