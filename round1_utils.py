...
def merge_decoy(original_msms_tab, 
    ...
    from_feat_pd = pd.read_csv(from_feat, sep='\t')
    from_feat_pd = from_feat_pd[from_feat_pd['Label']==-1]
    from_feat_pd = from_feat_pd.iloc[:sample_num]
    ...
    assert len(new_mr) == sample_num, f"Only {len(new_mr)} is found, expect {sample_num}"
