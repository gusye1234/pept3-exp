...
def spectrum(
    ...
    for i, (mz, intensity, annotation) in enumerate(zip(spec.mz, spec.intensity, annotations)):
        ...
        if i < (len(spec.mz)-1):
            ax.plot([mz, mz], [0, peak_intensity], color=color, zorder=zorder, linewidth=linewidth, alpha=alpha, solid_capstyle='round')
        else:
            ax.plot([mz, mz], [0, peak_intensity], color=color, zorder=zorder, linewidth=linewidth, alpha=alpha, solid_capstyle='round',label=label)
...
def mirror_multi(
    ...
    if spectrum_kws is None:
        spectrum_kws = {}
    # Top spectrum.
    for spec_top in spec_tops:
        spectrum(spec_top, mirror_intensity=False, ax=ax, **spectrum_kws)
    y_max = ax.get_ylim()[1]
    # Mirrored bottom spectrum.
    for spec_bottom in spec_bottoms:
        spectrum(spec_bottom, mirror_intensity=True, ax=ax, **spectrum_kws)
    y_min = ax.get_ylim()[0]
    ax.set_ylim(y_min, y_max)
