...
if not os.path.exists("HLA_cache.pkl"):
    def filter_HLA(file):
    ...
    del temp_DATA
    with open("HLA_cache.pkl", 'wb') as f:
        pickle.dump(DATA, f)
else:
    with open("HLA_cache.pkl", 'rb') as f:
        DATA = pickle.load(f)
...
if not os.path.exists("figs/fig2_sa_violin_alleles"):
    os.mkdir("figs/fig2_sa_violin_alleles")
RAW = True
print("Comparing raw spectra:", RAW, "which part", which_part_str)
if RAW:
    for which in Alleles:
        # for which in ['Mel-28_HLA-I']:
        print("-------------------------------")
        f_model_path = os.path.join('../checkpoints/finetuned/Alleles', which)
        print("load from", f_model_path)
        f_model1, f_model2 = pick_finetuned_model(f_model_path)
        f_tab = f"/data1/yejb/prosit/figure3/percolator_hdf5_allele_{set_threshold}/{which}"
        nf_tab = f"/data1/yejb/prosit/figure3/forPRIDE/Alleles/{which}/percolator"
        ...
        fig.savefig(
            f"figs/fig2_sa_violin_alleles/{frag_model}-{which_part_str}-{which}-raw.svg", dpi=300)
        plt.close()
else:
    for which in Alleles:
        # for which in ['Mel-28_HLA-I']:
        ...
        fig.savefig(
            f"figs/fig2_sa_violin_alleles/{frag_model}-{which_part_str}-{which}-ft.svg", dpi=300)
        plt.close()
