...
check_dir = "/data/yejb/prosit/figs/boosting/figs/Figure_5_HLA_2/forPride/rescoring_for_paper_2"
...
hla_mel = hla_mel[hla_mel['Experiment'].apply(lambda x: x.endswith("HLA-II"))]
naming_map = {
...
for MEL in mel2raw.keys():
    ...
    df = None
    for raw in mel2raw[MEL]:
        target = index_tab.filter(regex=f"{raw}$", axis=0)
        if df is None:
            df = target
        else:
...
maxquant_file = os.path.join(
    "/data/yejb/prosit/figs/boosting/figs/Figure_5_HLA_2/forPride/txt/msms.txt")
